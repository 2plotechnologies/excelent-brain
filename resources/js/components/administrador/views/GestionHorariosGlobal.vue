	...
	<div class="container-fluid p-2">
		...
		<div class="calendar-wrapper bg-white shadow-sm border" style="border-radius: 8px; height: calc(100vh - 180px); overflow-y: auto; overflow-x: hidden;" v-show="profesionalElegido">
			...
			<div class="calendar-header d-flex border-bottom bg-light" style="border-top-left-radius: 8px; border-top-right-radius: 8px;">
				...
				<!-- Cabecera Días (Eje X) -->
				<div class="doctors-header-container d-flex flex-grow-1" style="overflow-x: auto; overflow-y: hidden; min-width: 0;">
					<div class="doctor-header text-center py-2 border-right text-dark" v-for="dia in diasSemana" :key="dia">
						<div><strong class="mx-1 text-uppercase">{{ dia }}</strong></div>
					</div>
					<div class="doctor-header text-center py-2 text-dark bg-warning" style="min-width: 250px;">
						<div><strong class="mx-1 text-uppercase">Fechas Específicas</strong></div>
					</div>
				</div>
			</div>
			...
			<div class="calendar-body d-flex">
				...
				<!-- Columnas de días -->
				<div class="doctors-body-container d-flex flex-grow-1" style="overflow-x: auto; overflow-y: hidden; position: relative; min-width: 0;">
					<!-- Malla de fondo -->
					...
					<!-- Columnas para Lunes-Domingo (Recurrentes) -->
					<div class="doctor-column border-right position-relative" v-for="dia in diasSemana" :key="'col-'+dia" style="z-index: 1;">
						<div v-for="horario in getHorariosRecurrentesPorDia(dia)" :key="horario.id" 
								class="booked-slot shadow-sm p-1" 
								:style="[slotStyle(horario.check_time, horario.departure_date), { borderLeft: '4px solid ' + (horario.active ? '#1cc88a' : '#e74a3b') }]"
								@click="abrirDetallesHorario(horario)">
							<div class="booked-content h-100 position-relative overflow-hidden d-flex flex-column" :class="horario.active ? 'bg-success text-white' : 'bg-danger text-white'">
								<div class="font-weight-bold text-truncate lh-1" style="font-size: 0.75rem;">
									<i :class="horario.active ? 'fas fa-check-circle' : 'fas fa-times-circle'"></i> {{ horario.active ? 'Activo' : 'Inactivo' }}
								</div>
					...
					<!-- Columna para Fechas Específicas -->
					<div class="doctor-column position-relative bg-light" style="z-index: 1;">
						<div v-for="horario in horariosEspecificos" :key="'esp-'+horario.id" 
								class="booked-slot shadow-sm p-1" 
								:style="[slotStyle(horario.check_time, horario.departure_date), { borderLeft: '4px solid #f6c23e' }]"
								@click="abrirDetallesHorario(horario)">
							<div class="booked-content h-100 position-relative overflow-hidden d-flex flex-column bg-warning text-dark">
								<div class="font-weight-bold text-truncate lh-1" style="font-size: 0.75rem;">
									<i class="far fa-calendar-alt"></i> {{ fechaLatam(horario.date) }}
								</div>
								<div class="mt-1 text-truncate" style="font-size: 0.65rem; line-height: 1;">
									{{ formatHora(horario.check_time) }} - {{ formatHora(horario.departure_date) }}
								</div>
		...
		<div class="modal fade" id="modalDetallesHorario" tabindex="-1" aria-labelledby="modalDetallesHorarioLabel" aria-hidden="true" ref="modalDetalles">
			<div class="modal-dialog modal-sm">
				<div class="modal-content">
					...
					<div class="modal-body text-center" v-if="horarioSeleccionado">
						<p class="mb-2"><strong>Día/Fecha:</strong> {{ horarioSeleccionado.date ? fechaLatam(horarioSeleccionado.date) : horarioSeleccionado.day }}</p>
						...
						<div class="d-grid gap-2">
							<button class="btn" :class="horarioSeleccionado.active ? 'btn-warning' : 'btn-success'" @click="toggleActive(horarioSeleccionado.id)">
								<i :class="horarioSeleccionado.active ? 'fas fa-pause' : 'fas fa-play'"></i> 
								{{ horarioSeleccionado.active ? 'Desactivar Horario' : 'Activar Horario' }}
							</button>
							<button class="btn btn-outline-danger" @click="eliminarHorario(horarioSeleccionado.id)">
...
export default {
	...
	computed: {
		horariosEspecificos() {
			return this.horarios.filter(h => h.date !== null && h.date !== '');
		}
	},
	methods: {
		...
		async obtenerHorarios() {
			if (!this.profesionalElegido) return;
			try {
				const res = await this.axios.get(`/api/professional/${this.profesionalElegido}/schedules/all`);
				this.horarios = res.data;
			} catch (err) {
				console.error("Error cargando horarios", err);
			}
		},
		getHorariosRecurrentesPorDia(dia) {
			return this.horarios.filter(h => (h.date === null || h.date === '') && h.day === dia);
		},
		...
		async guardarHorario() {
			...
			if (this.tipoHorario === 'especifico' && this.nuevoHorario.date) {
				...
				let diaSemanaTarget = fechaSeleccionada.day(); // 0: Dom, 1: Lun...
